...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error.message?.includes("overloaded") || error.message?.includes("UNAVAILABLE") || error.status === 503) {
      errorMessage = "⏳ Das Modell ist momentan überlastet. Bitte warte 1-2 Minuten und versuche es erneut.";
    } else if (error.message?.includes("quota") || error.message?.includes("429")) {
      errorMessage = "API-Limit erreicht. Bitte versuche es später erneut oder aktiviere Pay-as-you-go in Google Cloud.";
